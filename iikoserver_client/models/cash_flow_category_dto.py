...
class CashFlowCategoryDto(BaseModel):
    ...
    CashFlowCategoryDto
    """ # noqa: E501
    id: StrictStr = Field(description="Идентификатор статьи ДДС")
    code: StrictStr = Field(description="Код статьи ДДС")
    parent_category_id: Optional[StrictStr] = Field(default=None, description="Родительская статья ДДС", alias="parentCategoryId")
    type: StrictStr = Field(description="Тип деятельности")
    __properties: ClassVar[List[str]] = ["id", "code", "parentCategoryId", "type"]
    
    # XML metadata for the class
    __xml_config: ClassVar[Dict[str, Any]] = {
        "root_name": "CashFlowCategoryDto",
    ...
    def type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['OPERATIONAL', 'INVESTMENT', 'FINANCE']):
            raise ValueError("must be one of enum values ('OPERATIONAL', 'INVESTMENT', 'FINANCE')")
        return value
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CashFlowCategoryDto from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CashFlowCategoryDto from a dict"""
        if obj is None:
            return None
    ...
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of CashFlowCategoryDto from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of CashFlowCategoryDto from an XML element"""
    ...
    def _from_xml_element_internal(cls, element: Element) -> Optional[Self]:
        ...
        # Process field: parent_category_id (XML name: parentCategoryId)
        # Handle XML element: parent_category_id
        # Single element field: parent_category_id
        child_element = element.find("parentCategoryId")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                data["parentCategoryId"] = text_value
    ...
    def _to_xml_element_internal(self, element: Element) -> None:
        ...
        # Process field: parent_category_id (XML name: parentCategoryId)
        value = getattr(self, "parent_category_id", None)
        if value is not None:  # Skip None values
            # Single element field: parent_category_id
            child_element = SubElement(element, "parentCategoryId")
            child_element.text = value
