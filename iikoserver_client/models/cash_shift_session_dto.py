...
class CashShiftSessionDto(BaseModel):
    """
    CashShiftSessionDto
    """ # noqa: E501
    session_id: StrictStr = Field(description="Id смены", alias="sessionId")
    group_id: StrictStr = Field(description="Id группы секций работающих в одной кассовой смене", alias="groupId")
    number: StrictInt = Field(description="Номер смены")
    __properties: ClassVar[List[str]] = ["sessionId", "groupId", "number"]
    
    # XML metadata for the class
    __xml_config: ClassVar[Dict[str, Any]] = {
        "root_name": "CashShiftSessionDto",
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of CashShiftSessionDto from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of CashShiftSessionDto from a dict"""
        if obj is None:
            return None
    ...
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of CashShiftSessionDto from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of CashShiftSessionDto from an XML element"""
    ...
    def _from_xml_element_internal(cls, element: Element) -> Optional[Self]:
        ...
        # Process field: group_id (XML name: groupId)
        # Handle XML element: group_id
        # Single element field: group_id
        child_element = element.find("groupId")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                data["groupId"] = text_value
                
        
        # Process field: number (XML name: number)
        # Handle XML element: number
        # Single element field: number
        child_element = element.find("number")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                try:
                    data["number"] = int(text_value)
                except ValueError:
                    data["number"] = text_value
    ...
    def _to_xml_element_internal(self, element: Element) -> None:
        ...
        # Process field: group_id (XML name: groupId)
        value = getattr(self, "group_id", None)
        if value is not None:  # Skip None values
            # Single element field: group_id
            child_element = SubElement(element, "groupId")
            child_element.text = value
            
        # Process field: number (XML name: number)
        value = getattr(self, "number", None)
        if value is not None:  # Skip None values
            # Single element field: number
            child_element = SubElement(element, "number")
