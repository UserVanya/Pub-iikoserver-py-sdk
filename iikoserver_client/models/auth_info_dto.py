...
class AuthInfoDto(BaseModel):
    ...
    AuthInfoDto
    """ # noqa: E501
    user: Optional[StrictStr] = Field(default=None, description="UUID пользователя")
    card: Optional[StrictStr] = Field(default=None, description="Номер карты")
    __properties: ClassVar[List[str]] = ["user", "card"]
    
    # XML metadata for the class
    __xml_config: ClassVar[Dict[str, Any]] = {
        "root_name": "AuthInfoDto",
    ...
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of AuthInfoDto from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of AuthInfoDto from an XML element"""
    ...
    def _from_xml_element_internal(cls, element: Element) -> Optional[Self]:
        ...
        # Process field: user (XML name: user)
        # Handle XML element: user
        # Single element field: user
        child_element = element.find("user")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                data["user"] = text_value
                
        
        # Process field: card (XML name: card)
        # Handle XML element: card
        # Single element field: card
        child_element = element.find("card")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                data["card"] = text_value
    ...
    def _to_xml_element_internal(self, element: Element) -> None:
        ...
        # Process field: user (XML name: user)
        value = getattr(self, "user", None)
        if value is not None:  # Skip None values
            # Single element field: user
            child_element = SubElement(element, "user")
            child_element.text = value
            
        # Process field: card (XML name: card)
        value = getattr(self, "card", None)
        if value is not None:  # Skip None values
            # Single element field: card
            child_element = SubElement(element, "card")
            child_element.text = value
