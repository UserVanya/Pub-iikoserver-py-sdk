...
class OlapV2Response(BaseModel):
    """
    OlapV2Response
    """ # noqa: E501
    data: List[Dict[str, Any]] = Field(description="Линейные данные отчета (построчно)")
    summary: List[Annotated[List[Dict[str, Any]], Field(min_length=2, max_length=2)]] = Field(description="Промежуточные и общие итоги по отчету")
    __properties: ClassVar[List[str]] = ["data", "summary"]
    
    # XML metadata for the class
    __xml_config: ClassVar[Dict[str, Any]] = {
        "root_name": "OlapV2Response",
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of OlapV2Response from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of OlapV2Response from a dict"""
        if obj is None:
            return None
    ...
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of OlapV2Response from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of OlapV2Response from an XML element"""
    ...
    def _from_xml_element_internal(cls, element: Element) -> Optional[Self]:
        ...
        child_elements = element.findall("data")
        items = []
        ...
        # Process field: summary (XML name: summary)
        # Handle XML element: summary
        # Array field: summary
        # Non-wrapped array - direct elements
        # Primitive items in non-wrapped array
        child_elements = element.findall("summary")
        items = []
        for child in child_elements:
            text_value = child.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
    ...
    def _to_xml_element_internal(self, element: Element) -> None:
        ...
        if value is not None:  # Skip None values
            ...
            for item in value:
                # Primitive items in non-wrapped array
                item_element = SubElement(element, "data")
                
                # Handle other primitive types in non-wrapped array items
                item_element.text = str(item)
                
        # Process field: summary (XML name: summary)
        value = getattr(self, "summary", None)
        if value is not None:  # Skip None values
            # Array field: summary
            # Non-wrapped array - add elements directly
            for item in value:
                # Primitive items in non-wrapped array
                item_element = SubElement(element, "summary")
