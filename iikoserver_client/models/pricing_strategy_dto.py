...
class PricingStrategyDto(BaseModel):
    """
    Стратегия расчета цены
    """ # noqa: E501
    type: PricingStrategyTypeEnum
    delta: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Абсолютное значение скидки/надбавки. Если знак '-', то скидка, если '+', то надбавка. Актуально для ABSOLUTE_VALUE")
    percent: Optional[Union[StrictFloat, StrictInt]] = Field(default=None, description="Значение скидки/надбавки в процентах. Если знак '-', то скидка, если '+', то надбавка. Диапазон значений: [-100, +inf). Актуально для PERCENT")
    __properties: ClassVar[List[str]] = ["type", "delta", "percent"]
    
    # XML metadata for the class
    __xml_config: ClassVar[Dict[str, Any]] = {
        "root_name": "PricingStrategyDto",
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of PricingStrategyDto from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of PricingStrategyDto from a dict"""
        if obj is None:
            return None
    ...
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of PricingStrategyDto from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of PricingStrategyDto from an XML element"""
    ...
    def _from_xml_element_internal(cls, element: Element) -> Optional[Self]:
        ...
        # Process field: delta (XML name: delta)
        # Handle XML element: delta
        # Single element field: delta
        child_element = element.find("delta")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                try:
                    data["delta"] = float(text_value)
                except ValueError:
                    data["delta"] = text_value
                
        
        # Process field: percent (XML name: percent)
        # Handle XML element: percent
        # Single element field: percent
        child_element = element.find("percent")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                try:
                    data["percent"] = float(text_value)
                except ValueError:
                    data["percent"] = text_value
    ...
    def _to_xml_element_internal(self, element: Element) -> None:
        ...
        # Process field: delta (XML name: delta)
        value = getattr(self, "delta", None)
        if value is not None:  # Skip None values
            # Single element field: delta
            child_element = SubElement(element, "delta")
            child_element.text = str(value)
            
        # Process field: percent (XML name: percent)
        value = getattr(self, "percent", None)
        if value is not None:  # Skip None values
            # Single element field: percent
            child_element = SubElement(element, "percent")
            child_element.text = str(value)
