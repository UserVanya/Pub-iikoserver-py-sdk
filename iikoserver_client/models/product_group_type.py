...
class ProductGroupType(str, Enum):
    """
    Типы групп продукта:  PRODUCTS - Продукт MODIFIERS - Модификатор (Используется только в номенклатуре, которая загружается и выгружается в/из RKeeper/StoreHouse)
    """
    ...
    def from_json(cls, json_str: str) -> Self:
        """Create an instance of ProductGroupType from a JSON string"""
        return cls(json.loads(json_str))

    @classmethod
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of ProductGroupType from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of ProductGroupType from an XML element"""
        if element is None:
            return None
