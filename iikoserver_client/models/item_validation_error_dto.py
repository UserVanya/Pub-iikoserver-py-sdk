...
class ItemValidationErrorDto(BaseModel):
    """
    ItemValidationErrorDto
    """ # noqa: E501
    identifier: StrictInt = Field(description="Номер элемента")
    error: List[ValidationErrorDto] = Field(description="Список ошибок элемента")
    __properties: ClassVar[List[str]] = ["identifier", "error"]
    
    # XML metadata for the class
    __xml_config: ClassVar[Dict[str, Any]] = {
        "root_name": "ItemValidationErrorDto",
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of ItemValidationErrorDto from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of ItemValidationErrorDto from a dict"""
        if obj is None:
            return None
        ...
        _obj = cls.model_validate({
            ...
            "error": [ValidationErrorDto.from_dict(_item) for _item in obj["error"]] if obj.get("error") is not None else None
        })
        return _obj
    ...
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of ItemValidationErrorDto from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of ItemValidationErrorDto from an XML element"""
    ...
    def _from_xml_element_internal(cls, element: Element) -> Optional[Self]:
        ...
        # Process field: identifier (XML name: identifier)
        # Handle XML element: identifier
        # Single element field: identifier
        child_element = element.find("identifier")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                try:
                    data["identifier"] = int(text_value)
                except ValueError:
                    data["identifier"] = text_value
                
        
        # Process field: error (XML name: error)
        # Handle XML element: error
        # Array field: error
        # Non-wrapped array - direct elements
        # Complex items in non-wrapped array
        child_elements = element.findall("error")
        items = []
    ...
    def _to_xml_element_internal(self, element: Element) -> None:
        ...
        if value is not None:  # Skip None values
            # Single element field: identifier
            child_element = SubElement(element, "identifier")
            child_element.text = str(value)
            
        # Process field: error (XML name: error)
        value = getattr(self, "error", None)
        if value is not None:  # Skip None values
            # Array field: error
            ...
            for item in value:
                ...
                if hasattr(item, "_to_xml_element_internal"):
                    item_element = SubElement(element, "error")
