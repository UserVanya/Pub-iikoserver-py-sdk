...
class BarcodeDto(BaseModel):
    ...
    BarcodeDto
    """ # noqa: E501
    barcode: Optional[StrictStr] = Field(default=None, description="Штрихкод.")
    container_id: Optional[StrictStr] = Field(default=None, description="UUID фасовки.", alias="containerId")
    __properties: ClassVar[List[str]] = ["barcode", "containerId"]
    
    # XML metadata for the class
    __xml_config: ClassVar[Dict[str, Any]] = {
        "root_name": "BarcodeDto",
    ...
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of BarcodeDto from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of BarcodeDto from an XML element"""
    ...
    def _from_xml_element_internal(cls, element: Element) -> Optional[Self]:
        ...
        # Process field: barcode (XML name: barcode)
        # Handle XML element: barcode
        # Single element field: barcode
        child_element = element.find("barcode")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                data["barcode"] = text_value
    ...
    def _to_xml_element_internal(self, element: Element) -> None:
        ...
        # Process field: barcode (XML name: barcode)
        value = getattr(self, "barcode", None)
        if value is not None:  # Skip None values
            # Single element field: barcode
            child_element = SubElement(element, "barcode")
