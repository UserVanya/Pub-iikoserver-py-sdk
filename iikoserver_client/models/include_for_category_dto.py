...
class IncludeForCategoryDto(BaseModel):
    """
    Включение продукта для ценовой категории
    """ # noqa: E501
    category_id: Optional[StrictStr] = Field(default=None, description="Идентификатор ценовой категории", alias="categoryId")
    include: Optional[StrictBool] = Field(default=None, description="Если true, то цену берем из PriceForCategoryDto, если false, то для данной ценовой категории данный продукт исключен из прайс-листа")
    __properties: ClassVar[List[str]] = ["categoryId", "include"]
    
    # XML metadata for the class
    __xml_config: ClassVar[Dict[str, Any]] = {
        "root_name": "IncludeForCategoryDto",
    ...
    def from_json(cls, json_str: str) -> Optional[Self]:
        """Create an instance of IncludeForCategoryDto from a JSON string"""
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        """Create an instance of IncludeForCategoryDto from a dict"""
        if obj is None:
            return None
    ...
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of IncludeForCategoryDto from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of IncludeForCategoryDto from an XML element"""
    ...
    def _from_xml_element_internal(cls, element: Element) -> Optional[Self]:
        ...
        # Process field: category_id (XML name: categoryId)
        # Handle XML element: category_id
        # Single element field: category_id
        child_element = element.find("categoryId")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                data["categoryId"] = text_value
                
        
        # Process field: include (XML name: include)
        # Handle XML element: include
        # Single element field: include
        child_element = element.find("include")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                data["include"] = text_value.lower() == 'true'
    ...
    def _to_xml_element_internal(self, element: Element) -> None:
        ...
        # Process field: category_id (XML name: categoryId)
        value = getattr(self, "category_id", None)
        if value is not None:  # Skip None values
            # Single element field: category_id
            child_element = SubElement(element, "categoryId")
            child_element.text = value
            
        # Process field: include (XML name: include)
        value = getattr(self, "include", None)
        if value is not None:  # Skip None values
            # Single element field: include
            child_element = SubElement(element, "include")
