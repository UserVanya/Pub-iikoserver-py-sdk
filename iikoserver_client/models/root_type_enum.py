...
class RootTypeEnum(str, Enum):
    """
    Тип справочных данных: Account - Счет (в том числе склады) AccountingCategory - Бухгалтерская категория номенклатуры AlcoholClass - Класс алкогольной продукции AllergenGroup - Группа аллергенов AttendanceType - Тип явки сотрудника Conception - Концепция CookingPlaceType - Тип места приготовления DiscountType - Тип скидки MeasureUnit - Единица измерения OrderType - Тип заказа PaymentType - Тип оплаты ProductCategory - Пользовательская категория номенклатуры ProductScale - Шкала размеров ProductSize - Размер продукта ScheduleType - Тип смены TaxCategory - Налоговая категория
    """
    ...
    def from_json(cls, json_str: str) -> Self:
        """Create an instance of RootTypeEnum from a JSON string"""
        return cls(json.loads(json_str))

    @classmethod
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of RootTypeEnum from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of RootTypeEnum from an XML element"""
        if element is None:
            return None
