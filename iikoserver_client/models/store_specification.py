...
class StoreSpecification(BaseModel):
    """
    Структура используется для указания на подмножество подразделений, в которых действует содержащая ее строка техкарты.
    """ # noqa: E501
    departments: Optional[List[StrictStr]] = Field(default=None, description="Список ID подразделений.")
    inverse: Optional[StrictBool] = Field(default=None, description="false — фильтр является включающим (строка действует для всех перечисленных подразделений). true — фильтр является исключающим (строка действует для всех подразделений, КРОМЕ перечисленных, включая подразделения, созданные после последнего сохранения техкарты).")
    __properties: ClassVar[List[str]] = ["departments", "inverse"]
    
    # XML metadata for the class
    __xml_config: ClassVar[Dict[str, Any]] = {
        "root_name": "StoreSpecification",
        
        
        "field_configs": {
            "departments": {
                "xml_name": "departments",
    ...
    def from_xml(cls, xml_str: str) -> Optional[Self]:
        """Create an instance of StoreSpecification from an XML string"""
        try:
    ...
    def from_xml_element(cls, element: Element) -> Optional[Self]:
        """Create an instance of StoreSpecification from an XML element"""
    ...
    def _from_xml_element_internal(cls, element: Element) -> Optional[Self]:
        ...
        # Process field: departments (XML name: departments)
        # Handle XML element: departments
        # Array field: departments
        # Non-wrapped array - direct elements
        # Primitive items in non-wrapped array
        child_elements = element.findall("departments")
        items = []
        ...
        # Process field: inverse (XML name: inverse)
        # Handle XML element: inverse
        # Single element field: inverse
        child_element = element.find("inverse")
        if child_element is not None:
            # Check for null or empty values
            text_value = child_element.text
            if text_value is not None and text_value.strip() != "" and text_value != "null":
                data["inverse"] = text_value.lower() == 'true'
    ...
    def _to_xml_element_internal(self, element: Element) -> None:
        ...
        # Process field: departments (XML name: departments)
        value = getattr(self, "departments", None)
        if value is not None:  # Skip None values
            # Array field: departments
            # Non-wrapped array - add elements directly
            for item in value:
                # Primitive items in non-wrapped array
                item_element = SubElement(element, "departments")
                item_element.text = item
                
        # Process field: inverse (XML name: inverse)
        value = getattr(self, "inverse", None)
        if value is not None:  # Skip None values
            # Single element field: inverse
            child_element = SubElement(element, "inverse")
